...
public class Messages {
	...
	public static EmbedBuilder noPermission() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark.getValue() + " **I was unable to perform that action for you.**");
	}
	
	public static EmbedBuilder invalidUserType() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark.getValue() + " **Invalid user type. (\"bot\" or \"client\")**");
	}
	
	public static EmbedBuilder unableToFindMentionedMessage() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark.getValue() + " **Unable to find mentioned message in the mentioned channel.**");
	}
	
	public static EmbedBuilder noMentionedMessage() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark.getValue() + " **No message mentioned.**");
	}
	
	public static EmbedBuilder noMentionedChannel() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark.getValue() + " **No channel mentioned.**");
	}
	
	public static EmbedBuilder noMentionedRole() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark.getValue() + " **No role mentioned.**");
	}
	
	public static EmbedBuilder noMentionedRoles() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark.getValue() + " **No roles mentioned.**");
	}
	
	public static EmbedBuilder reactionSetupExpired() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark.getValue() + " **Reaction role setup expired.**");
	}
	
	public static EmbedBuilder failedToApplyReactionRole(String reason) {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark + " **Failed to apply reaction role to message!**\n`" + reason + "`");
	}
	
	public static EmbedBuilder invalidReactionCommandFormat() {
		return getEmbedFrame().setColor(Color.RED).setDescription(Emoji.CrossMark 
				+ " **Invalid command format. (\"" + GateKeeper.getInstance().getPrefix() + "reactionrole #TaggedChannel NumericMessageID @MentionedRole\")");
	}
	...
	public static EmbedBuilder reactionRoleReactionRequiredMessage() {
		return getEmbedFrame().setColor(Color.GREEN).setDescription(Emoji.GreenCheck + " **Reaction role message credentials verified!** "
